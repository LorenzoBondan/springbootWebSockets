...
function onClickButton(){ // button
    ...
    stompClient.send("/app/chat.componentClicked", {}, JSON.stringify({sender: username, type: 'SYSTEM', content: `${currentDatetime} - ${username} clicked in the button`}))
    connectingElement.classList.add('hidden');
}
...
function onClickTextBox(){ // textbox
    var currentDatetime = new Date().toLocaleString();
    stompClient.subscribe('/topic/public');
    stompClient.send("/app/chat.componentClicked", {}, JSON.stringify({sender: username, type: 'SYSTEM', content: `${currentDatetime} - ${username} clicked in the textbox`}))
    connectingElement.classList.add('hidden');
}
...
function onCheckCheckBox(){ // checkbox
    ...
    if (isChecked) {
        stompClient.subscribe('/topic/public');
        stompClient.send("/app/chat.componentClicked", {}, JSON.stringify({sender: username, type: 'SYSTEM', content: `${currentDatetime} - ${username} checked the checkbox`}))
        connectingElement.classList.add('hidden');
    }
    else{
        stompClient.subscribe('/topic/public');
        stompClient.send("/app/chat.componentClicked", {}, JSON.stringify({sender: username, type: 'SYSTEM', content: `${currentDatetime} - ${username} unchecked the checkbox`}))
        connectingElement.classList.add('hidden');
    }
}
...
function onChangeSelectItem(){ // select (combobox)
    ...
    stompClient.subscribe('/topic/public');
    stompClient.send("/app/chat.componentClicked", {}, JSON.stringify({sender: username, type: 'SYSTEM', content: `${currentDatetime} - ${username} changed select item to ${selectedItem}`}))
    connectingElement.classList.add('hidden');
}
...
function onMouseHoverLabel(){
    var currentDatetime = new Date().toLocaleString();
    stompClient.subscribe('/topic/public');
    stompClient.send("/app/chat.componentClicked", {}, JSON.stringify({sender: username, type: 'SYSTEM', content: `${currentDatetime} - ${username} hover the mouse on label`}))
    connectingElement.classList.add('hidden');
}
